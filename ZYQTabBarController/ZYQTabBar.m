...
#define kZYQDefaultTintColor             [UIColor blackColor]
#define kZYQDefaultItemHighlightColor    [UIColor colorWithWhite:1.f alpha:0.2f]
...
- (CGFloat)dimensionToBeConsideredOfItem:(ZYQTabBarItem *)item;

@end


@implementation ZYQTabBar
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _layoutStrategy = ZYQTabBarLayoutStrategyStrungTogether;
        _itemPadding = 0.f;
        _position = kZYQTabBarPositionDefault;
...
- (void)layoutSubviews {
    ...
    if (self.layoutStrategy == ZYQTabBarLayoutStrategyEvenlyDistributed || self.layoutStrategy == ZYQTabBarLayoutStrategyCentered) {
        // compute total dimension needed
        for (ZYQTabBarItem *item in self.items) {
            ...
            if (self.layoutStrategy != ZYQTabBarLayoutStrategyEvenlyDistributed) {
        ...
        if (self.layoutStrategy == ZYQTabBarLayoutStrategyEvenlyDistributed) {
            ...
            CGFloat totalPadding = ZYQTabBarIsVertical(self.position) ? self.bounds.size.height - totalDimension : self.bounds.size.width - totalDimension;
    ...
    ZYQTabBarItem *lastItem = [self.items lastObject];
...
- (void)setItems:(NSArray *)items {
    if (items != _items) {
        ...
        for (ZYQTabBarItem *item in _items) {
...
- (void)setPosition:(ZYQTabBarPosition)position {
...
- (void)selectItemAtIndex:(NSUInteger)index {
    ...
    if (index < self.items.count) {
        ZYQTabBarItem *item = [self.items objectAtIndex:index];
        item.selected = YES;
...
- (void)deselectSelectedItem {
    if (self.selectedItemIndex < self.items.count) {
        ZYQTabBarItem *selectedItem = [self.items objectAtIndex:self.selectedItemIndex];
...
- (UIColor *)tintColor {
    return _tintColor ?: kZYQDefaultTintColor;
}
...
- (UIColor *)itemHighlightColor {
    return _itemHighlightColor ?: kZYQDefaultItemHighlightColor;
}
...
- (void)updateItemHighlight {
    if (self.selectedItemIndex != NSNotFound) {
        ...
        self.itemHighlightView.frame = ZYQTabBarIsVertical(self.position) ? CGRectInset(itemRect, 2.f, 0.f) : CGRectInset(itemRect, 0.f, 2.f);
