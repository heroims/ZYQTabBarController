...
#define kZYQDefaultTintColor                 [UIColor colorWithRed:41.0/255.0 green:147.0/255.0 blue:239.0/255.0 alpha:1.0]
#define kZYQDefaultTitleColor                [UIColor lightGrayColor]
#define kZYQDefaultSelectedTitleColor        [UIColor whiteColor]
#define kZYQImageOffset                       5.f

@interface ZYQTabBarItem () {
...
+ (ZYQTabBarItem *)itemWithTitle:(NSString *)title image:(UIImage *)image {
    ZYQTabBarItem *item = [[ZYQTabBarItem alloc] initWithFrame:CGRectZero];
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _titleLabel.textColor = kZYQDefaultTitleColor;
...
- (void)layoutSubviews {
    ...
    if (self.image != nil) {
        CGFloat imageOffset = self.title.length > 0 ? kZYQImageOffset : 0.f;
...
- (UIColor *)selectedImageTintColor {
    return _selectedImageTintColor ?: kZYQDefaultTintColor;
}

- (UIColor *)titleColor {
    return _titleColor ?: kZYQDefaultTitleColor;
}

- (UIColor *)selectedTitleColor {
    return _selectedTitleColor ?: kZYQDefaultSelectedTitleColor;
}
...
@implementation UIViewController (ZYQTabBarItem)

- (void)zyq_setTabBarItem:(ZYQTabBarItem *)zyq_tabBarItem {
    objc_setAssociatedObject(self, &itemKey, zyq_tabBarItem, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (ZYQTabBarItem *)zyq_tabBarItem {
    return objc_getAssociatedObject(self, &itemKey);
}

- (ZYQTabBarController *)zyq_tabBarController {
    return (ZYQTabBarController *)objc_getAssociatedObject(self, kZYQTabBarControllerKey);
}
